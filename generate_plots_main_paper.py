...
cols_exp_id = ["f96dd7", "13496586", "13439788", "13439790"]
cols_accept_reject = [True, False, True, True]
num_of_runs = 8


fig, ax = plt.subplots(2, 4, figsize=(6.9, 2.7))
fig.tight_layout(w_pad=0.2, h_pad=0.4, rect=(0.01, 0.01, 0.998, 0.83))
...
for exp_i, (experiment_id, accept_reject) in enumerate(zip(cols_exp_id, cols_accept_reject)):
    ...
    for j in range(num_of_methods):
        line, = ax[0, exp_i].plot(np.linspace(1, T + 1, T), mean_rewards[j, :, chosen_plots[0]],
                                  color=colors_to_use[j], zorder=10*(num_of_methods - j))
    ...
    title = f'{dataset}, {"dynamic" if is_dynamic else "static"},\n ' \
            f'{"contextual" if structure == "CX" else "low-rank"}, ' \
            f'{"with" if accept_reject else "without"} rejections,\n'\
            f'$N$ = {N}, {f"$n$ = {N // 4}, " if is_dynamic else ""}$M$ = {M}, $R$ = {R}'
    ax[0, exp_i].set_title(title, x=0.48, y=1.0, pad=4)
...
plt.savefig(f"plots/pdfs/exp_main.pdf")
plt.savefig(f"plots/exp_main.jpeg")
